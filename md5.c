...
unsigned char *md5(unsigned char *M, md5_size len, unsigned char *_digest) {
  ...
  do {
    memcopy(context->buf + context->size, M + context->bits,
            buflen - context->size);
...
static void md5_encode(unsigned char *buffer, struct md5_ctx *context) {
  ...
  unsigned int a = context->regs.A, b = context->regs.B, c = context->regs.C,
               d = context->regs.D;
  unsigned int x[16];
...
/* OBSOLETE */
static void md5_memcopy(unsigned char *dest, unsigned char *src,
                        unsigned int count) {
...
static void md5_memset(unsigned char *p, const unsigned char c,
